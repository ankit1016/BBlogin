 ...
 */
function fromFileName(fullPath, pathDepth, pathSkip, shortenUnderscores, removeExtension, convertPathToSnakeCase) {
...
function getSubdirectoryPrefix(subfolderPrefixDefs) {
 ...
 */
function createDirectives(fileUri) {
    const config = common_1.getConfig(fileUri);
    const macroType = config.get("Macro Type", "GUID");
    const macroPrefix = config.get("Prefix", "");
    const macroSubfolderPrefixDefs = config.get("Subfolder Prefixes", []);
    ...
    const convertPathToSnakeCase = config.get("File Path Pascal Case to Snake Case", false);
    let macroName;
    if (macroType === "Filename") {
        macroName = fromFileName(false, pathDepth, pathSkip, shortenUnderscores, removeExtension, convertPathToSnakeCase);
    }
    else if (macroType === "Filepath") {
        macroName = fromFileName(true, pathDepth, pathSkip, shortenUnderscores, removeExtension, convertPathToSnakeCase);
    }
    else {
        macroName = fromGUID(preventDecimal);
    }
    const macroSubfolderPrefix = getSubdirectoryPrefix(macroSubfolderPrefixDefs);
...
function insertIncludeGuard() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const config = common_1.getConfig(editor.document.uri);
